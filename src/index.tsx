...
function ButtonCalc({ value, handleClick, row }: { value: string, handleClick: Function, row: string[] }) {
  return (
    <button 
...
function ButtonGroupCalc({ handleClick }: { handleClick: Function }) {
  const values = [['%', 'CE', 'C', '⌫'],
                  ['1/x', '√','^', '/'],
                  ['7', '8', '9', '*'], 
                  ['4', '5', '6', '-'], 
...
function DisplayScreen({ display, label }: { display: string, label: string }) {
  return (
...
export default function Calculator() {
  ...
  function toFixedNecessary(value: any, n: number){
...
function handleClick({value}: {value: string}) {
  function handleOperator({operator, firstValue, setDisplay, setFirstValue, setLabel, value}: 
      {operator: string | null, firstValue: number, setDisplay: Function, setFirstValue: Function, setLabel: Function, value: string}) {
  ...
  if (specialOperators.includes(value)) {
    switch (value) {
      ...
      case '√':
        if (parseFloat(display) >= 0) {
          setDisplay(prevDisplay => String(toFixedNecessary(Math.sqrt(parseFloat(prevDisplay)), 12)));
        }
        break;
      case '%':
        if (parseFloat(display) >= 0) {
          setDisplay(prevDisplay => String(toFixedNecessary(parseFloat(prevDisplay) / 100, 12)));
        }
        break;
      case '1/x':
        if (parseFloat(display) !== 0 && display) {
          setDisplay(prevDisplay => String(toFixedNecessary(1 / parseFloat(prevDisplay), 12)));
        }
        break;
      default:
        break;
    ...
    if (operator && !display) {
      ...
      setOperator(value);
      handleOperator({operator, firstValue, setDisplay, setFirstValue, setLabel, value});
    }
  } else if (value === '=') {
